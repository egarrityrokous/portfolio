...
if __name__ == '__main__':
    ...
    test = sys.argv[2]
    try:
        decrypt('data/de.allowed.txt.encrypted', crypto_key)
        decrypt('data/de.possible.txt.encrypted', crypto_key)
        decrypt('data/el.allowed.txt.encrypted', crypto_key)
        decrypt('data/el.possible.txt.encrypted', crypto_key)
        call_result = subprocess.call(['pytest', '-s', '--tb=short', '-rN', 'test.py::' + test])
    except Exception as e:
        print(e)
        call_result = 1
    finally:
        if os.path.exists("data/de.allowed.txt"):
            os.remove("data/de.allowed.txt")
        if os.path.exists("data/de.possible.txt"):
            os.remove("data/de.possible.txt")
        if os.path.exists("data/el.allowed.txt"):
            os.remove("data/el.allowed.txt")
        if os.path.exists("data/el.possible.txt"):
            os.remove("data/el.possible.txt")
    sys.exit(call_result)